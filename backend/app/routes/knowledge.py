...
async def upload_knowledge_document(
    ...
    connector_id = await _ensure_knowledge_connector(email, companyId)
    now = _now()
    document_id = str(uuid.uuid4())
    company_dir = KNOWLEDGE_STORAGE_DIR / companyId
    company_dir.mkdir(parents=True, exist_ok=True)
    storage_path = company_dir / f"{document_id}-{filename}"
    ...
    return {"success": True, "document": _serialize(doc), "connectorId": connector_id}
