...
SurfaceKind = Literal["web", "openapi", "api_docs", "document_url", "file", "knowledge_note", "code_repository", "code_file"]
ExpectedSurface = Literal["web", "api", "documents", "code", "email", "database", "other"]
IcaBenchmarkModeKind = str
...
class IcaSolutionDiscoveryEvaluation(BaseModel):
    ...
    solutionCount: int
    extraSolutionTaskIds: list[str] = Field(default_factory=list)
    missingTaskIds: list[str] = Field(default_factory=list)
    incompleteTaskIds: list[str] = Field(default_factory=list)
    invalidOriginIds: list[str] = Field(default_factory=list)
    hallucinatedToolNames: list[str] = Field(default_factory=list)
    hallucinatedConnectorIds: list[str] = Field(default_factory=list)
    incompleteReasons: dict[str, list[str]] = Field(default_factory=dict)
    solutions: list[IcaTaskSolutionSpec] = Field(default_factory=list)


class IcaAgentExecutionTaskResult(BaseModel):
    taskId: str
    passed: bool
    score: float
    agentId: str = ""
    buildPassed: bool = True
    buildErrors: list[str] = Field(default_factory=list)
    agentConfigSummary: dict[str, Any] = Field(default_factory=dict)
    executedTools: list[str] = Field(default_factory=list)
    assertions: list[dict[str, Any]] = Field(default_factory=list)
    state: dict[str, Any] = Field(default_factory=dict)
    error: str = ""


class IcaAgentExecutionEvaluation(BaseModel):
    projectId: str
    mode: IcaBenchmarkModeKind | None = None
    phase: Literal["agent_execution"] = "agent_execution"
    applicable: bool = False
    skippedReason: str = ""
    executionMode: Literal["none", "trajectory_replay_harness", "runtime_agent"] = "none"
    runtimeExecuted: bool = False
    passed: bool = True
    score: float = 1.0
    expectedTaskCount: int = 0
    executedTaskCount: int = 0
    passedTaskIds: list[str] = Field(default_factory=list)
    failedTaskIds: list[str] = Field(default_factory=list)
    results: list[IcaAgentExecutionTaskResult] = Field(default_factory=list)
