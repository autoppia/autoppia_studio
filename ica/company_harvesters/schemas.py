...
CompanyCodeLanguage = Literal["python", "typescript", "javascript"]
...
InventoryOrigin = Literal[
    "existing",
    "existing_connector_tool",
...
class CompanyHarvesterInput(BaseModel):
    ...
    discoveryMode: Literal["ui_only", "ui_api", "ui_code", "api_code", "ui_api_docs", "code_only", "full_company"] = "full_company"
    userTasks: list[dict[str, Any]] = Field(default_factory=list)
    runtimeKinds: list[AgentRuntimeKind] = Field(default_factory=lambda: ["model_agent", "codex", "claude_code"])
    availableInventory: dict[str, Any] = Field(default_factory=lambda: {"connectors": [], "tools": [], "runtimes": []})
    metadata: dict[str, Any] = Field(default_factory=dict)


class CompanyCodeFile(BaseModel):
    path: str
    content: str


class CompanyCustomConnectorCode(BaseModel):
    language: CompanyCodeLanguage = "python"
    entrypoint: str = ""
    files: list[CompanyCodeFile] = Field(default_factory=list)
    authRequirements: list[str] = Field(default_factory=list)
...
class CompanyCustomToolCode(BaseModel):
    language: CompanyCodeLanguage = "python"
    functionName: str = ""
    code: str = ""
    tests: list[str] = Field(default_factory=list)
...
class CompanyConnectorPlan(BaseModel):
    connectorId: str = ""
    name: str = ""
    type: Literal["web", "api", "knowledge", "code", "email", "database", "custom"] = "custom"
    origin: InventoryOrigin = "unknown"
    existingConnectorId: str = ""
    surface: str = ""
    authRequired: bool = False
    runtimeRequirements: list[str] = Field(default_factory=list)
    evidence: list[dict[str, Any]] = Field(default_factory=list)
    customConnectorCode: CompanyCustomConnectorCode | None = None
    metadata: dict[str, Any] = Field(default_factory=dict)
...
class CompanyToolPlan(BaseModel):
    ...
    evidence: list[dict[str, Any]] = Field(default_factory=list)
    customToolCode: CompanyCustomToolCode | None = None
    metadata: dict[str, Any] = Field(default_factory=dict)
